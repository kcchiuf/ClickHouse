    ...
    const S3ObjectStorageSettings & settings)
{
    ...
    UInt64 attempts = config.getUInt64(config_prefix + ".retry_attempts", settings.request_settings.retry_attempts);
    client_configuration.retryStrategy = std::make_shared<Aws::Client::DefaultRetryStrategy>(std::max(1ul, attempts) - 1);
